...
export default function BuilderClient({
  ...
  const openEdit = (s: IHomepageLayout) => {
    ...
    setCategoryId(s.categoryId ? ((s.categoryId as any)._id?.toString() || s.categoryId.toString()) : "");
    ...
    <div className="bg-white p-6 rounded-xl border border-gray-200">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-gray-800">Homepage Layout Builder</h2>
        </div>
        {canCreate && (
          <Button
      ...
      <p className="text-sm text-gray-500 mb-6">
        Drag sections to reorder how content appears on the public homepage. Each section maps to a visual block.
      </p>

      {sections.length === 0 ? (
        <div className="text-center p-12 border border-dashed rounded-xl text-gray-500">
          No homepage sections configured. Click "Add Section" to start building.
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-2 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  ...
                  <span className="bg-gray-100 px-2 py-0.5 rounded">{s.layoutType}</span>
                  {s.isPinned && (
                    <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded">Pinned</span>
                  )}
                </div>
              </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {canUpdate && (
                    <>
                      ...
                      <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => moveSection(idx, "down")} disabled={idx === sections.length - 1}>
                        <ArrowDown className="w-4 h-4" />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => handleToggleEnabled(s)}>
                        {s.enabled ? <Eye className="w-4 h-4 text-green-600" /> : <EyeOff className="w-4 h-4 text-gray-400" />}
                      </Button>
                  ...
                  {canDelete && (
                    <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => handleDelete(s._id.toString())}>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingId ? "Edit Section" : "Add Homepage Section"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-1.5">
              <Label>Section Name *</Label>
              <Input placeholder="e.g. Politics Grid, Sports Highlights" value={sectionName} onChange={(e) => setSectionName(e.target.value)} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" value={postsCount} onChange={(e) => setPostsCount(parseInt(e.target.value) || 6)} />
              </div>
            </div>
            <div className="flex gap-6">
              <div className="flex items-center space-x-2">
                <Checkbox id="sec-enabled" checked={enabled} onCheckedChange={(v) => setEnabled(!!v)} />
                <Label htmlFor="sec-enabled" className="cursor-pointer">Enabled</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox id="sec-pinned" checked={isPinned} onCheckedChange={(v) => setIsPinned(!!v)} />
                <Label htmlFor="sec-pinned" className="cursor-pointer">Pinned</Label>
              </div>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={isSubmitting}>
              {isSubmitting ? "Saving..." : editingId ? "Save Changes" : "Create Section"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
