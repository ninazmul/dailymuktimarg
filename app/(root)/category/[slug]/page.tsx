    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {subCats.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-6">
          {subCats.map((sc: any) => (
            <Link key={sc._id} href={`/category/${sc.slug}`} className="text-xs bg-gray-100 text-gray-700 px-3 py-1.5 rounded-full hover:bg-primary/10 hover:text-primary transition font-medium">
              {sc.name}
            </Link>
          ))}
        </div>
      ...
      {safeArticles.length === 0 ? (
        <div className="text-center p-12 text-gray-500">No articles found in this category.</div>
      ) : (
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-8">
          {page > 1 && (
            <Link href={`/category/${slug}?page=${page - 1}`} className="px-4 py-2 text-sm bg-white border rounded-lg hover:bg-gray-50">
              ← Previous
            </Link>
          )}
